...
def index(request):
    ...
    return render_to_response('hiphopathy/index.html', {'artist_list': artist_list})
    
def search_form(request):
    return render_to_response('hiphopathy/search_form.html')
    
def search(request):
    if 'q' in request.GET and request.GET['q']:
        q = request.GET['q']
        snippet_list = Snippet.objects.filter(songid = q)
        return render_to_response('hiphopathy/search_results.html',
...
def song_view(request, artistid):
    ...
    return render_to_response('hiphopathy/song_index.html', {'song_list': song_list})
    
    
def snippet_view(request, songid):
    chunk_list = Snippet.objects.filter(songid = songid)
    return render_to_response('hiphopathy/snippet_index.html', {'chunk_list': chunk_list})
    

def post(request):
	if 'user_supplied_answer' in request.GET and request.GET['user_supplied_answer']:
		a = request.GET['user_supplied_answer']
		return render_to_response('hiphopathy/snippet_post.html', {'answer': user_supplied_answer})
	else:
